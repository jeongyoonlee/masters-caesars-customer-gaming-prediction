...
def train_predict(train_file, test_file, feature_map_file, predict_valid_file, predict_test_file,
    ...
    logging.info('prepare load CV features')
    ...
    for i, (i_trn, i_val) in enumerate(cv.split(y), 1):
        ...
        logging.info('loading features for CV #{}'.format(i))
        X_trn_cv, _ = load_data('{}.trn{}{}'.format(feature_name, i, feature_ext))
        X_tst_cv, _ = load_data('{}.tst{}{}'.format(feature_name, i, feature_ext))

        X_trn_full = np.concatenate((X, X_trn_cv), axis=1)
        X_tst_full = np.concatenate((X_tst, X_tst_cv), axis=1)

        lgb_trn = lgb.Dataset(X_trn_full[i_trn], y[i_trn])
        lgb_val = lgb.Dataset(X_trn_full[i_val], y[i_val])
